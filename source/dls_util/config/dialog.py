from PyQt4.QtGui import QVBoxLayout, QGroupBox, QDialog
...
class ConfigDialog(QDialog):
    ...
    def _make_dialog_buttons(self):
        ...
        assert (isinstance(btn_cancel.pressed, pyqtBoundSignal))
        btn_cancel.pressed.connect(self._dialog_close_cancel)
        btn_ok = QPushButton("OK")
        assert (isinstance(btn_ok.pressed, pyqtBoundSignal))
        btn_ok.pressed.connect(self._dialog_close_ok)
        btn_apply = QPushButton("Apply")
        assert (isinstance(btn_apply.pressed, pyqtBoundSignal))
        btn_apply.pressed.connect(self._dialog_apply_changes)
        btn_reset = QPushButton("Defaults")
        assert (isinstance(btn_reset.pressed, pyqtBoundSignal))
